...
It is extremely unlikely that the code you write will run without generating errors. Some of the errors are difficult to comprehend. Error messages in R are infamous for their uselessness. They are often written by advanced users for advanced users. For us they often are impossible to comprehend. Google is an invaluable resource to find solutions, because R users are very active and happy to assist each other. But you may not find what you are looking for right away. So you need to be persistent.
...
To find out where R has currently set as your working directory, you write this code: `getwd()`.

When I **run** the above code, R shows me what my current working directory is.
```{r}
getwd()
```
